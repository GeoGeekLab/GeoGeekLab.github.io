...
function solarDirection(date = new Date()) {
  ...
  let subsolarLon = 15 * (12 - hour - eqMinutes / 60);
  subsolarLon = ((subsolarLon + 540) % 360) - 180;
  return worldPoint(declination * 180 / Math.PI, subsolarLon, 1).normalize();
}

function makeTerminator(sunDirection) {
  const axis = Math.abs(sunDirection.y) < .9 ? new THREE.Vector3(0, 1, 0) : new THREE.Vector3(1, 0, 0);
  const u = new THREE.Vector3().crossVectors(sunDirection, axis).normalize();
  ...
  const material = new THREE.LineBasicMaterial({ color: 0xd8ddd8, transparent: true, opacity: .22, depthWrite: false });
  const line = new THREE.Line(geometry, material);
  line.renderOrder = 3;
  line.name = 'solar-terminator';
  return line;
}
...
export class GeoOrbitalField {
  constructor(options) {
    ...
    this.sunDirection = solarDirection();
    this.lastSolarUpdate = 0;
  ...
  initScene() {
    ...
    this.terminatorLine = makeTerminator(this.sunDirection);
    this.worldGroup.add(this.terminatorLine);
  ...
  updateSolar(force = false) {
    const now = Date.now();
    if (!force && now - this.lastSolarUpdate < 60000) return;
    this.lastSolarUpdate = now;
    this.sunDirection = solarDirection(new Date(now));
    if (this.terminatorLine) {
      this.worldGroup.remove(this.terminatorLine);
      this.terminatorLine.geometry.dispose();
      this.terminatorLine.material.dispose();
    }
    this.terminatorLine = makeTerminator(this.sunDirection);
    this.worldGroup.add(this.terminatorLine);
  }
  ...
  installDemoField() {
    ...
    this.demo = true;
    this.records = makeDemoRecords(count, this.locale);
  ...
  select(index) {
    ...
    const geo = formatCoordFromXYZ(x, y, z, this.locale);
    const record = this.records[index];
    const light = new THREE.Vector3(x, y, z).normalize().dot(this.sunDirection) >= 0 ? 'daylight' : 'night';
    this.onSelect({ index, record, ...geo, light });
  ...
  pick(selectOnHit = false) {
    ...
    if (satHits.length && Number.isInteger(satHits[0].instanceId)) {
      ...
      const o = index * 3;
      const geo = formatCoordFromXYZ(this.positions?.[o] || 0, this.positions?.[o + 1] || 0, this.positions?.[o + 2] || 0, this.locale);
...
export async function mountOrbitalLab({ container, locale = 'en', signal, labels = {}, statusCallback = null }) {
  ...
  const inclination = container.querySelector('#orbitalLabInclination');
  const epoch = container.querySelector('#orbitalLabEpoch');
  const type = container.querySelector('#orbitalLabType');
  const light = container.querySelector('#orbitalLabLight');
